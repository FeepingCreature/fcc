...
class WithStmt : Namespace, Statement, ScopeLike {
  ...
  this(Expr ex) {
    ...
    if (auto onUsing = iparse!(Statement, "onUsing", "tree.semicol_stmt.expr")("eval ex.onUsing", "ex", ex)) {
      pre = stuple(pre, onUsing) /apply/ (typeof(pre) pre, Statement st, AsmFile af) { if (pre) pre(af); st.emitAsm(af); };
    }
    if (auto onExit = iparse!(Statement, "onExit", "tree.semicol_stmt.expr")("eval ex.onExit", "ex", ex)) {
